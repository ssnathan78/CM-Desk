...
import { chaseSlHitSummary, chaseSlTrailSlack, chaseSlTrailSummary } from "../chaseCopy"
...
async function persistChaseSlHitExit(args: {
  nfoSymbol: string
  tradingsymbol: string
  status: string
  stoploss: number
  side: "BUY" | "SELL"
  cover: "working_stop" | "market_flatten"
  paperBook: boolean
}) {
  const summary = chaseSlHitSummary({
    status: args.status,
  ...
  await recordStrategySignal({
    ...
    outcome: "EXIT",
    summary,
    features: { status: args.status, stoploss: args.stoploss, cover: args.cover },
    idempotencyKey: `chase:slhit:${args.nfoSymbol}:${toIst(dayjs()).format("YYYY-MM-DDTHH:mm")}`,
  })
  await recordDecision({
    ...
    side: args.side,
    action: "EXIT",
    intent: `COVER ${args.side} @ ${args.stoploss}`,
    reason: summary,
    proposedQty: undefined,
    proposedPrice: args.stoploss,
    idempotencyKey: `chase-exit:${args.tradingsymbol}:${args.side}:${toIst(dayjs()).format("YYYY-MM-DDTHH:mm")}`,
    provenance: args.paperBook ? "PAPER" : "LIVE",
  })
}
...
async function processUpdateSLForInstrument(job: Job, nfoSymbol: string) {
  ...
  if (
    ...
    const paperStopsFilled = await matchPaperWorkingStops({
      tradingsymbol,
      last: candle.close,
      high: candle.high,
      low: candle.low,
