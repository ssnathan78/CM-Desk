...
export async function listOrders(limitOrQuery: number | TradeListQuery = 100) {
  ...
  return db.select().from(orders).where(bookClause).orderBy(desc(orders.updatedAt)).limit(limit)
}
...
export async function listTrades(query: number | TradeListQuery = 100) {
  ...
  if (opts.from || opts.to) {
    ...
    const entered: SQL =
      from && to
        ? (and(gte(trades.entryAt, from), lte(trades.entryAt, to)) as SQL)
        : from
          ? gte(trades.entryAt, from)
          : lte(trades.entryAt, to as Date)
    const exited: SQL =
      from && to
        ? (and(isNotNull(trades.exitAt), gte(trades.exitAt, from), lte(trades.exitAt, to)) as SQL)
        : from
          ? (and(isNotNull(trades.exitAt), gte(trades.exitAt, from)) as SQL)
          : (and(isNotNull(trades.exitAt), lte(trades.exitAt, to as Date)) as SQL)
    clauses.push(or(entered, exited) as SQL)
  }
