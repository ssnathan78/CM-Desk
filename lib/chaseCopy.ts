...
export function chaseSlHitSummary(input: {
  status: string
  tradingsymbol: string
  stoploss: number
  ...
  const how =
    input.cover === "working_stop" ? "working SL filled" : "flatten MARKET after SL level tagged"
  return `SL hit — covered ${input.status} ${input.tradingsymbol} at ${input.stoploss} (${how})`
}
