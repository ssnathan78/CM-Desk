...
# From a laptop (after ~/.ssh/config Host cm-desk-prod):
...
echo "=== cm-desk production health (read-only) ==="
...
if have systemctl; then
  for unit in cm-desk nginx caddy postgresql redis-server redis docker; do
...
elif [[ -f /opt/cm-desk/.env ]]; then
  ENV_FILE=/opt/cm-desk/.env
fi
...
if have curl; then
  code=$(curl -sS -o /tmp/cm-desk-health.json -w "%{http_code}" --max-time 8 "http://127.0.0.1:3000/api/health" || echo "000")
  echo "GET /api/health http_status=${code}"
  if [[ -f /tmp/cm-desk-health.json ]]; then
    python3 - <<'PY' 2>/dev/null || cat /tmp/cm-desk-health.json
import json
try:
    data = json.load(open("/tmp/cm-desk-health.json", encoding="utf-8"))
...
if have docker; then
  docker exec cm-desk-postgres pg_isready -U postgres -d trading_db >/dev/null 2>&1 \
...
if have docker; then
  docker exec cm-desk-redis redis-cli ping >/dev/null 2>&1 \
