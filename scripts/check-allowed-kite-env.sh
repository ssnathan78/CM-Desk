...
docker exec cm-desk-app node -e "const v=process.env.ALLOWED_KITE_USER_ID; console.log(v && String(v).trim() ? 'CONTAINER=configured' : 'CONTAINER=missing')"
