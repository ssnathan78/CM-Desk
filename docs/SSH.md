...
| Item | Where it lives |
|---|---|
| Host alias | Laptop `~/.ssh/config` → `Host cm-desk-prod` (Windows: `C:\Users\<you>\.ssh\config`, **no `.txt`**) |
| Linux user | `cmdesk` (sudo needs a password; Docker does not) |
| Key | `~/.ssh/id_ed25519` — **passphrase-protected** |
| Droplet layout | Docker at `/srv/cmdesk`; nginx :80/:443; app **3000 on 127.0.0.1 only** |
| App containers | `cm-desk-app`, `cm-desk-postgres`, `cm-desk-redis` |
...
4. Public key (`id_ed25519.pub`) is in `/home/cmdesk/.ssh/authorized_keys` on the Droplet.
...
```
Production SSH host alias is cm-desk-prod (defined only in the operator laptop ~/.ssh/config).
Always: ssh -o BatchMode=yes cm-desk-prod -- <command>
...
```powershell
ssh -o BatchMode=yes cm-desk-prod -- uname -a
ssh -o BatchMode=yes cm-desk-prod -- "docker ps"
ssh -o BatchMode=yes cm-desk-prod -- "curl -sS -m 8 http://127.0.0.1:3000/api/health"
```
...
- `cmdesk` can run `docker` without sudo. `sudo` needs the operator’s password — the agent cannot enter it.
...
|---|---|
| `Could not resolve hostname cm-desk-prod` | No `~/.ssh/config`, or Notepad saved `config.txt` |
...
```powershell
ssh -vvv -o BatchMode=yes cm-desk-prod -- uname -a
```
