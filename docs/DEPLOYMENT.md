...
| App tree | `/srv/cmdesk` |
| Containers | `cm-desk-app`, `cm-desk-postgres`, `cm-desk-redis` |
| Image name | `cm-desk-app` (Compose project `cm-desk` + service `app`, production target) |
| Proxy | Host **nginx** → `127.0.0.1:3000` |
| App port | Bind **localhost only** (`127.0.0.1:3000:3000`) |
| systemd unit | **Not used** (`cm-desk.service` / `/opt/cm-desk` are the alternate install) |
...
Workflow: [`.github/workflows/ci.yml`](../.github/workflows/ci.yml). **Does not run on push.** Trigger it manually from [Actions](https://github.com/ssnathan78/CM-Desk/actions) if you want a hosted lint/test/build/e2e pass. Production deploys are the Droplet only.
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- 'docker ps --format "table {{.Names}}\t{{.Status}}"'
```
...
| **C. systemd, no Docker** | Fresh VM you install that way (not this Droplet) | On the VM (`yarn build`) | `/opt/cm-desk/.env` + unit file |
...
```bash
ssh -o BatchMode=yes cm-desk-prod
cd /srv/cmdesk
...
docker logs --tail 80 cm-desk-app
```
...
# Compose names this image cm-desk-app. Confirm:
docker images --format "{{.Repository}}:{{.Tag}}" | Select-String "cm-desk"
```

Tag if the local name differs:

```powershell
docker tag cm-desk-app:latest cm-desk-app:latest
```
...
```powershell
docker save cm-desk-app:latest | ssh -o BatchMode=yes cm-desk-prod "docker load"
```
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- 'cd /srv/cmdesk && docker compose up -d --no-deps app'
ssh -o BatchMode=yes cm-desk-prod -- 'curl -fsS -m 8 http://127.0.0.1:3000/api/health'
ssh -o BatchMode=yes cm-desk-prod -- 'docker logs --tail 80 cm-desk-app'
```

Optional: `git pull` on `/srv/cmdesk` **without** replacing `docker-compose.yml` / `.env`, so the tree matches the image for debugging.
...
You need on the server: Node 22, Yarn 4.9.1 (`corepack enable`), PostgreSQL 16, Redis 7, this git repo, a reverse proxy for HTTPS, and `deploy/cm-desk.service`.

```bash
sudo mkdir -p /opt/cm-desk
sudo git clone https://github.com/ssnathan78/CM-Desk.git /opt/cm-desk
cd /opt/cm-desk
yarn install --immutable
yarn build
yarn migrate
sudo cp deploy/cm-desk.service /etc/systemd/system/
# Edit WorkingDirectory / EnvironmentFile if your path differs
sudo systemctl daemon-reload
sudo systemctl enable --now cm-desk
sudo systemctl status cm-desk
```

`deploy/cm-desk.service` runs `yarn start` (`node --require ./otel.js server.js`). Logs: `journalctl -u cm-desk -f`.

### New version (systemd)

```bash
cd /opt/cm-desk
sudo systemctl stop cm-desk
git pull
yarn install --immutable
yarn build
yarn migrate          # read the SQL if new files appeared
sudo systemctl start cm-desk
curl -fsS https://YOUR_HOST/api/health
```

### Rollback (systemd)

Keep the previous git commit id. `git checkout THAT_SHA && yarn build && systemctl restart cm-desk`. If a **forward** migration already ran, do not check out old code on a new schema unless you have a tested down-migration.

---

## Environment on the server

Copy `.env.example` to `/srv/cmdesk/.env` (Docker) or `/opt/cm-desk/.env` (systemd). Never commit it.
...
```
15 16 * * 1-5 /opt/cm-desk/scripts/backup-db.sh
```

On Docker, run dump via `docker compose exec postgres` against `cm-desk-postgres` (do not log the password). Copy dumps **off** the Droplet. A snapshot + dump is better than dump alone.

## Docker rollback

1. Note `docker inspect cm-desk-app --format '{{.Config.Image}}'` and `git -C /srv/cmdesk rev-parse HEAD`.
2. Load the previous `cm-desk-app` image (or `git checkout` + rebuild).
