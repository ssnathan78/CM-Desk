...
Last audit: **2026-09-05** (read-only SSH as `cmdesk@cm-desk-prod`). No services were restarted. No `.env` values were printed.
...
$h | ssh -o BatchMode=yes cm-desk-prod "bash -s"
```
...
| Check | Result |
|---|---|
| SSH `BatchMode` to `cm-desk-prod` | OK (agent must hold the key) |
...
| Install style | **Docker Compose**, not systemd `cm-desk.service` |
| App dir on host | `/srv/cmdesk` |
...
| Container image | locally built `cm-desk-app` (Dockerfile `production` target) |
| App process | `cm-desk-app` Up **21h (healthy)**, restart count **0** |
| `GET /api/health` | **200** `{ status: ok, service: cm-desk }` (older payload; no `checks` object) |
| Postgres | `cm-desk-postgres` **postgres:16-alpine**, healthy, `pg_isready` OK; **5432 not published** on the host |
| Redis | `cm-desk-redis` **redis:7-alpine**, healthy, `PONG`; **6379 not published** on the host |
...
Internet
  ...
  → cm-desk-app (Node in Docker, workers in-process)
       → cm-desk-postgres (Postgres 16, Docker network only)
       → cm-desk-redis (Redis 7, Docker network only)
       → Kite (outbound)
```

Not used on this box: Caddy, `/opt/cm-desk`, systemd unit `cm-desk`.

## Known problems / gaps

1. **Port 3000 was reachable from the internet** (Docker bypassed UFW). **Fixed 2026-09-05:** Compose `app` ports set to `127.0.0.1:3000:3000`; `cm-desk-app` recreated. nginx HTTPS still 200. Leave `app-dev` profile mapping unchanged.
...
8. **sudo on `cmdesk` requires a password**, so the agent cannot read `ufw`/`sshd -T` without you. Run those yourself if you want the remaining SSH/firewall rows.
