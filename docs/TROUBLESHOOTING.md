...
Kite itself succeeded; saving the session cookie failed. The cookie library rejects names that contain `/`. The cookie is now `cmdesk-kite-session`. Rebuild/restart the app and click **Continue with Kite** once.
