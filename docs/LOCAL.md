...
This is the path if you are running CM-Desk on **your PC** with Docker Desktop. You will log in to **real Kite**, see **live prices**, but **not** send live orders (`MOCK_ORDERS=true`). Production can do the same per strategy: Desk → Risk execution **Paper** uses live quotes and writes the ledger, but never calls Kite `placeOrder`.
...
```powershell
cd C:\senthil
git clone https://github.com/ssnathan78/CM-Desk.git
cd CM-Desk
```
...
**Recommended:** create a **second** Kite Connect app named something like “CM-Desk local” and point only that app at `127.0.0.1`. Keep production keys on the server.
...
|---|---|---|
| `cm-desk-postgres` | 127.0.0.1:5432 | Database |
| `cm-desk-redis` | 127.0.0.1:6379 | BullMQ / cache |
| `cm-desk-app` | 127.0.0.1:3000 | Website + workers |
