...
Any coding agent or human terminal (same Windows user)
    ...
    → Droplet sshd :22
    → cmdesk (docker without sudo; sudo needs a password)
...
| Host alias | `Host cm-desk-prod` in **laptop** `~/.ssh/config` | Example only: `docs/ssh-config.example` |
...
Least privilege: a deploy user that can `systemctl restart cm-desk` (or docker) via sudoers, not daily root SSH.

## Secrets

| Secret | Where it should live | Never |
|---|---|---|
| `.env` | Server only (`/opt/cm-desk/.env` or compose env) | Git, chat, health-script stdout |
...
After `ssh cm-desk-prod` works, collect (do not “fix” in the same breath):
...
| Item | Observation |
|---|---|
| SSH | Key auth for `cmdesk`; laptop key is passphrase-protected; agent required for `BatchMode` |
| sudo | `cmdesk` is in `sudo` and `docker`; sudo **needs a password** (agent cannot elevate) |
