...
3. **This repo (CM-Desk)** — `lib/chaseSignal.ts`, `lib/queue-processor/chaseQueue.ts`, `lib/ema.ts`, plus ATM straddle/strangle under `lib/strategies/`.
...
**CM-Desk is a close automation of the published Chase rule book**, not of chase-bot’s STRATEGY.md.
...
If you treat chase-bot STRATEGY.md as the floor, CM-Desk will look “over-specified.” If you treat the published rule book as the floor, chase-bot is the outlier.
...
| chase-bot claim | Rule book | CM-Desk | Severity of chase-bot vs rule book |
|---|---|---|---|
...
**Do not “fix” CM-Desk to match chase-bot STRATEGY.md.** That would delete the actual Chase playbook. If Python chase-bot is still running anywhere, treat it as a **different, simpler strategy**.

Pine snippet in the operator markdown plots T1 SL at 09:15 and signal bands — it does **not** encode pending day-high entries. CM-Desk followed the **article text and annotated charts**, which is the right source.

The rule book’s **prose** for short T+1 copies the long template (`EMA < close < Short T1`), which is **impossible** (Short T1 is below EMA). The **charts** and CM-Desk use the mirror: `shortT1 ≤ close ≤ EMA` → midpoint of EMA and **previous day high**. Trust the charts + this code, not that one sentence.
