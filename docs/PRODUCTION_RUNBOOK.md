...
SSH is **laptop OpenSSH** + host alias `cm-desk-prod`. Full setup, Windows `ssh-agent` reminder, and how **any** coding agent connects: [SSH.md](./SSH.md). Template: [ssh-config.example](./ssh-config.example).

```bash
ssh -o BatchMode=yes cm-desk-prod -- uname -a
```

If that host is missing, stop. Do not invent `HostName` or `User`.

**This Droplet (2026-09):** Docker at `/srv/cmdesk` (`cm-desk-app` / `cm-desk-postgres` / `cm-desk-redis`). nginx on 80/443. App port **3000 is localhost-only**. There is no systemd `cm-desk.service`.
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- 'whoami; hostname; pwd'
```
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- 'systemctl is-active cm-desk; systemctl status cm-desk --no-pager'
```

Docker (if that is how it was installed):

```bash
ssh -o BatchMode=yes cm-desk-prod -- 'docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"'
```

Health (localhost on the Droplet; does not print secrets):

```bash
ssh -o BatchMode=yes cm-desk-prod -- 'curl -sS -m 8 -o /tmp/h.json -w "%{http_code}\n" http://127.0.0.1:3000/api/health; python3 -c "import json;print(json.load(open(\"/tmp/h.json\")))"'
```
...
```bash
ssh -o BatchMode=yes cm-desk-prod "bash -s" < scripts/production-health-check.sh
```
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- 'ps aux | grep -E "[n]ode|[y]arn" | grep -v grep'
```

Do **not** print Redis `bull:*` key names (they include `KITE_API_KEY`). Count only:

```bash
ssh -o BatchMode=yes cm-desk-prod -- 'redis-cli --scan --pattern "bull:*:meta" | wc -l'
```
...
```bash
# systemd
ssh -o BatchMode=yes cm-desk-prod -- 'journalctl -u cm-desk -n 200 --no-pager'

# docker
ssh -o BatchMode=yes cm-desk-prod -- 'docker logs --tail 200 cm-desk-app'

# proxy (paths vary; do not assume)
ssh -o BatchMode=yes cm-desk-prod -- 'sudo journalctl -u nginx -n 100 --no-pager'
ssh -o BatchMode=yes cm-desk-prod -- 'sudo journalctl -u caddy -n 100 --no-pager'
```
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- 'pg_isready -h 127.0.0.1 -p 5432'
ssh -o BatchMode=yes cm-desk-prod -- 'ls /opt/cm-desk/drizzle/*.sql'
```
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- 'redis-cli ping'
```
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- 'sudo nginx -t'
ssh -o BatchMode=yes cm-desk-prod -- 'systemctl is-active nginx caddy'
PUBLIC_HOST=your.domain ssh -o BatchMode=yes cm-desk-prod -- "PUBLIC_HOST=your.domain bash -s" < scripts/production-health-check.sh
```
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- 'df -h; free -h; uptime; nproc'
```
...
```bash
# systemd
sudo systemctl restart cm-desk
...
This Droplet is **Docker** at `/srv/cmdesk`. CI does **not** deploy.
...
```bash
ssh -o BatchMode=yes cm-desk-prod -- <read-only>
```
