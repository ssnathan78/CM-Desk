...
Behavior-driven automated tests for cm-desk. Goal: catch bugs a real operator would hit, not maximize line coverage alone.
...
Runs on GitHub-hosted Ubuntu only when you start the workflow by hand ([Actions](https://github.com/ssnathan78/CM-Desk/actions)). **Does not deploy.** Push to `master` does not start it. Public repo: standard runners are free.
